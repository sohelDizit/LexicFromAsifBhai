...
namespace ASC.Projects.Data.DAO
{
    ...
    class ProjectDao : BaseDao, IProjectDao
    {
        ...
        private readonly Converter<object[], Project> converterDescription;


        public ProjectDao(int tenantId) : base(tenantId)
        {
            converter = ToProject;
            converterDescription = ToProjectWithDescription;
        }
        ...
        public Project GetProjectNumber(int projectId)
        {
            string[] colmn = new[] { "id", "ProjectNumber" };
            var product= Db.ExecuteList(Query(ProjectsTable).Select(colmn).Where("id", projectId))
                .ConvertAll(converterDescription)
                .SingleOrDefault();
            return product;
        }
        ...
        public Project Create(Project project)
        {
            var insert = Insert(ProjectsTable, false)
                .InColumns("id", "title", "description", "status", "responsible_id", "private", "create_by", "create_on", "last_modified_on", "ProjectNumber")
                .Values(
                    project.ID,
                    project.Title,
                    project.Description,
        ...
        public static Project ToProject(object[] r)
        {
            return new Project
            {
                ...
                //ProjectNumber = (string)r[10],

            };
        }


        public static Project ToProjectWithDescription(object[] r)
        {
            return new Project
            {
                ID = Convert.ToInt32(r[0]),
