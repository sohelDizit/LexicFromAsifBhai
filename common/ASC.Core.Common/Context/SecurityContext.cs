...
namespace ASC.Core
{
    public static class SecurityContext
    {
        ...
        public static void SetUserPasswordHash(Guid userID, string passwordHash)
        {
           ...
           var prePasswords= CoreContext.Authentication.GetLastUserPasswordHistory(tenantid, user.Email, 6);

            if (prePasswords.Contains(passwordHash)) 
            {
                throw new PasswordException("You can't reuse 6 or more previous passwords");
            }


            CoreContext.Authentication.InsertPasswordHistory(tenantid, user.Email, passwordHash);
