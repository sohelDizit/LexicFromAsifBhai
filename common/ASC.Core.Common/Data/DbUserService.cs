...
namespace ASC.Core.Data
{
    public class DbUserService : DbBaseService, IUserService
    {
        ...
        public void InsertPasswordHistory(int tenant, string email, string passwordHash)
        {
            var i = Insert("core_password_history", tenant)
                .InColumnValue("Email", email)
                .InColumnValue("password", passwordHash)
                .InColumnValue("Submission_Date", DateTime.Now);
            ExecNonQuery(i);
        }

        public List<string> GetLastUserPasswordHistory(int tenant, string email, int limit)
        {
            var q = new SqlQuery("core_password_history")
                        .Select("ID","password")
                        .Where("Email", email).OrderBy(1,false).SetMaxResults(limit);
            if (tenant != Tenant.DEFAULT_TENANT)
            {
                q.Where("tenant", tenant);
            }
            ...
            return result.Select(p => p.Item2).ToList();
        }
